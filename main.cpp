...
/*
void benchmarkSearches(const vector<Song>& songs, Trie& trie, HashTable& hashTable) {
    using namespace chrono;
    const int N = 100;  // test first 100 songs (enough for comparison)

    long long trieTotal = 0, hashTotal = 0;

    for (int i = 0; i < N && i < (int)songs.size(); ++i) {
        string name = songs[i].getName();

        auto t1 = high_resolution_clock::now();
        trie.containsExact(name);
        auto t2 = high_resolution_clock::now();
        trieTotal += duration_cast<microseconds>(t2 - t1).count();

        t1 = high_resolution_clock::now();
        hashTable.searchName(name);
        t2 = high_resolution_clock::now();
        hashTotal += duration_cast<microseconds>(t2 - t1).count();
    }

    cout << "Average Search Times (across " << N << " songs):\n";
    cout << "Trie:       " << trieTotal / N << " microseconds\n";
    cout << "HashTable:  " << hashTotal / N << " microseconds\n";
*/
}
...
/*
        ...
        //benchmarkSearches(songs, trie, hashTable);
    }
